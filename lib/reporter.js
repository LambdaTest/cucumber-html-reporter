...
var stageRerunCount = getStageReRunCount();
...
var generateReport = function (options) {
  ...
  try {
    packageJson =
      packageJsonPath && jsonFile.readFileSync(packageJsonPath, "utf8");
  } catch (err) {
    console.warn(
      "No package.json file found in: " +
        packageJsonPath +
        ", using default name and version."
    );
  ...
  var suite = {
    name: {
      ...
      sanitized: sanitize(
        options.name || (packageJson && packageJson.name),
        /[^a-z|0-9]/g
      ),
    },
  ...
  var calculateDuration = function (durationInNanoSeconds) {
    ...
    function format(min, sec, ms) {
      ...
      return formattedTimeStamp.trim().length === 0
        ? "< 1ms"
        : formattedTimeStamp;
    }
  ...
  var setStats = function (suite) {
    ...
    if (options.screenshotsDirectory) {
      ...
      screenshotsDirectory = options.output
        ? path.join(options.output, "..", "screenshots")
        : "screenshots";
    }
    ...
    featureOutput.forEach(function (feature) {
      feature.hierarchy = hierarchyReporter.getFeatureHierarchy(
        feature.uri,
        basedir
      );
      feature.scenarios = {};
      ...
      feature.elements.forEach(function (element) {
        ...
        element.steps.forEach(function (step, count) {
          ...
          } else if (step.after !== undefined) {
            // iterate over after hooks
            step.after.forEach(function (hook) {
              if (hook.embeddings !== undefined) {
                hook.embeddings.forEach(function (embedding) {
                  handleEmbedding(
                    embedding,
                    step,
                    options,
      ...
      if (options.theme === "hierarchy") {
        subSuite = hierarchyReporter.findOrCreateSubSuite(
          suite,
          feature.hierarchy
        );
      }
      ...
      if (featuresSummary.isFailed) {
        ...
        subSuite
          ? hierarchyReporter.recursivelyIncrementStat(subSuite, "failed")
          : suite.failed++;
      } else if (featuresSummary.isAmbiguous) {
        featuresSummary.ambiguous++;
        subSuite
          ? hierarchyReporter.recursivelyIncrementStat(subSuite, "ambiguous")
          : suite.ambiguous++;
      } else if (feature.scenarios.count === feature.scenarios.skipped) {
        featuresSummary.skipped++;
        subSuite
          ? hierarchyReporter.recursivelyIncrementStat(subSuite, "skipped")
          : suite.skipped++;
      } else if (feature.scenarios.count === feature.scenarios.notdefined) {
        featuresSummary.notdefined++;
        subSuite
          ? hierarchyReporter.recursivelyIncrementStat(subSuite, "passed")
          : suite.passed++;
    ...
    Object.keys(featuresSummary || {}).forEach((key) => {
      if (!isNaN(featuresSummary[key] && featuresKeys.includes(key))) {
        featureTotal += featuresSummary[key];
      }
    });

    suite.features.summary.featureTotal = featureTotal;
  ...
  function readFile(fileName) {
    function getPath(name) {
      ...
      if (
        options.templateDir &&
        fs.existsSync(path.join(options.templateDir, name))
      ) {
...
function handleEmbedding(
  ...
  } else if (embeddingType === "image/url") {
    step.image = embedding.data;
  } else {
...
function generate(options, callback) {
  const reportType = path.extname(options.output);
  ...
  async function launchReport() {
    if (
      ...
      (options.launchReport || options.launchReport === "true")
    ) {
  ...
  if (reportType === ".json") {
    options.jsonFile = options.output;
  } else {
...
function getStageReRunCount() {
  ...
  var metaFile = process.env.META_FILE;
  try {
    var jobReports = jsonFile.readFileSync(metaFile);
    retryCount = 0;
    if (
      !!jobReports &&
      ...
      jobReports["JobSummary"]["Tasks"].forEach((task) => {
        if (
          ...
          !!task["stages"] &&
          !!task["stages"]["retried"] &&
          !isNaN(task["stages"]["retried"])
        ) {
          // +<num> makes sure that a numeric variable is getting read as a number
