...
const stageRerunCount = getStageReRunCount();

const generateReport = function (options) {
    const featureOutput = jsonFile.readFileSync(options.jsonFile);
    const packageJsonPath = searchFileUp('package.json');
    let packageJson = {};
    try {
                        ...
                        step.embeddings.forEach(function (embedding) {
                            ...
                            if (['text/plain', 'text/html', 'application/json'].includes(embeddingType)) {
                                ...
                                if (options.storeScreenshots && options.storeScreenshots === true) {
                                    ...
                                    const name = sanitize(step.name || step.keyword, /[^a-zA-Z0-9-]+/g); // Only allow URL-friendly file names
