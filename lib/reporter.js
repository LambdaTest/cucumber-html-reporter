                ...
                element.steps.forEach(function (step, count) {
                    if (step.embeddings !== undefined) {
                        step.embeddings.forEach(function (embedding) {
                            handleEmbedding(embedding, step, options, sanitize, screenshotsDirectory);
                        });
                    }
...
function handleEmbedding(embedding, step, options, sanitize, screenshotsDirectory) {
    var embeddingType = {};
...
function generate(options, callback) {
    ...
    if (string.EqualFold(options.reportType, "json")) {
    ...
    if (isValidJsonFile()) {
        if (string.EqualFold(options.reportType, "html")){
...
function getStageReRunCount() {
    ...
    try {
        ...
        retryCount = 0;
        if (!!jobReports && !!jobReports["JobSummary"] && !!jobReports["JobSummary"]["Tasks"] && jobReports["JobSummary"]["Tasks"] instanceof Array) {
