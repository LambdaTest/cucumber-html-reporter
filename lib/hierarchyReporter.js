 ...
 */
const getBaseDir = function (suite) {
    const baseDir = [];
    suite.features.forEach(function (feature) {
        const uriParts = feature.uri.split(path.sep);
        if (!baseDir.length) {
            baseDir = uriParts;
        } else {
            for (const i = uriParts.length; i > 0; i--) {
 ...
 */
const getFeatureHierarchy = function (featureUri, baseDir) {
    const noBaseDir = featureUri.slice(baseDir.length + 1);
    const noBadChars = noBaseDir.split('=').join('_');
    const featureDirs = noBadChars.split(path.sep);
 ...
 */
const findOrCreateSubSuite = function (suite, hierarchy) {
    ...
    const subSuiteName = hierarchy[0];
    if (!suite.suites) {
        suite.suites = [];
    }
    const subSuite = suite.suites.find(function (s) {
 ...
 */
const recursivelyIncrementStat = function (subSuite, attrName) {
